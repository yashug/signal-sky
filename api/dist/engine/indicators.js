 ...
 */
export function currentEma200(bars) {
    if (bars.length === 0)
        return NaN;
 ...
 */
export function allTimeHigh(bars) {
    if (bars.length === 0)
        return NaN;
 ...
 */
export function avgVolume(bars, period = 20) {
    if (bars.length === 0)
        return 0;
    const recent = bars.slice(-period);
 ...
 */
export function preResetATH(bars) {
    if (bars.length === 0)
        return NaN;
    ...
    if (resetIdx <= 0)
        return allTimeHigh(bars); // No reset found, use overall ATH
 ...
 */
export function distanceToBreakout(currentPrice, ath) {
    if (ath <= 0)
        return Infinity;
