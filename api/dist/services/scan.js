...
export class ScanService {
     ...
     */
    async scanSymbol(symbol, fromStr, today) {
        try {
     ...
     */
    async scanUniverse(symbols, concurrency = 20) {
        ...
        for (let i = 0; i < symbols.length; i += concurrency) {
            ...
            const batchResults = await Promise.all(batch.map((symbol) => this.scanSymbol(symbol, fromStr, today)));
