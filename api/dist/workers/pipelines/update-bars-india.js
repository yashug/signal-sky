...
export async function updateBarsIndia() {
    ...
    for (let i = 0; i < matched.length; i++) {
        ...
        try {
            ...
            const data = await kc.getHistoricalData(instToken, "day", formatDate(from), formatDate(to));
            if (data.length === 0)
                continue;
            const payload = data
                .filter((c) => c.date && Number.isFinite(c.open) && Number.isFinite(c.close))
                .map((c) => ({
                symbol,
                exchange: "NSE",
...
//# sourceMappingURL=update-bars-india.js.map