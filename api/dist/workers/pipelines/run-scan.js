 ...
 */
async function persistResult(result, todayDate, universes, prisma) {
    ...
    if (result.signal) {
        ...
        try {
            ...
            console.log(`  ${heat} ${sig.symbol.padEnd(18)} ${sig.heat.padEnd(10)} dist: ${sig.distanceToBreakout.toFixed(1)}%  vol: ${sig.volumeSurge.toFixed(1)}x`);
        }
        catch (e) {
 ...
 */
export async function runScanPipeline(market, provider) {
    ...
    const symbolUniverses = {};
    for (const m of marketMembers) {
        if (!symbolUniverses[m.symbol])
            symbolUniverses[m.symbol] = [];
    ...
    for (let i = 0; i < results.length; i += PERSIST_BATCH) {
        ...
        const batchResults = await Promise.all(batch.map((r) => persistResult(r, todayDate, symbolUniverses[r.symbol] ?? [], prisma)));
        for (let j = 0; j < batch.length; j++) {
            ...
            for (const univ of univs) {
                if (!healthCounts[univ])
                    healthCounts[univ] = { above: 0, total: 0 };
                healthCounts[univ].total++;
                if (pr.aboveEma)
                    healthCounts[univ].above++;
            }
        }
    }
    ...
    // Composite indices
    const compositeKeys = ["nifty100", "nifty200", "niftymidcap100", "niftysmallcap100"];
...
//# sourceMappingURL=run-scan.js.map