...
if (mode === "india") {
    ...
    runScanPipeline("IN")
        ...
        if (r.errors.length > 0)
            console.warn(`  Errors: ${r.errors.length}`);
    ...
    cron.schedule("30 10 * * 1-5", () => {
        runIndiaPipeline().catch((e) => console.error("[Cron] India pipeline failed:", e.message));
    }, { timezone: "UTC" });
    // US: 2:00 AM IST = 20:30 UTC prev day, Mon-Fri US market = Tue-Sat IST
    cron.schedule("30 20 * * 1-5", () => {
        runUSPipeline().catch((e) => console.error("[Cron] US pipeline failed:", e.message));
    }, { timezone: "UTC" });
    // Keep process alive
...
//# sourceMappingURL=cron-pipeline.js.map