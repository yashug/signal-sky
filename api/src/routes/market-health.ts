...
export async function marketHealthRoutes(app: FastifyInstance) {
  ...
  app.get("/market-health/history", async (request) => {
    ...
    const universeKeys = query.universes
      ? query.universes.split(",") as UniverseGroupKey[]
    ...
    for (const r of rows) {
      ...
      if (!dateMap.has(dateStr)) {
        const entry: Record<string, number | null> = { date: dateStr as any }
        for (const k of universeKeys) entry[k] = null
