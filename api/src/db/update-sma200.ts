 ...
 */
export async function updateEma200ForSymbol(symbol: string, exchange: string): Promise<number> {
  const prisma = getPrisma()

  const result: [{ update_ema200_for_symbol: number }] = await prisma.$queryRawUnsafe(
 ...
 */
export async function updateSma200ForSymbol(symbol: string, exchange: string): Promise<number> {
  const prisma = getPrisma()
 ...
 */
export async function updateMovingAveragesForSymbol(symbol: string, exchange: string): Promise<void> {
  await Promise.all([
