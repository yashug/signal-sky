...
import type { ApiBacktestSummaryRow, ApiBacktestsResponse } from "@/lib/api"
...
export function BacktestsClient({
  ...
  const backtests = useMemo(() => {
    if (universe === "all") return data.backtests
    const tags = new Set(resolveUniverseTags(universe))
    return data.backtests.filter((b) => {
      const memberTags = universeMemberships[b.symbol]
      return memberTags?.some((t) => tags.has(t))
    })
  }, [data.backtests, universe, universeMemberships])
